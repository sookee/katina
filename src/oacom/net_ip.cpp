...
static int net_enabled = 3; // ipv4 & ipv6 ?

static bool net_socksEnabled = false;
static str net_socksServer;
static int net_socksPort = 1080;
static str net_socksUsername;
static str net_socksPassword;
//
static str net_ip = "0.0.0.0";
static str net_ip6 = "::";
static int net_port = PORT_SERVER;
static int net_port6 = PORT_SERVER;
static str net_mcast6addr;
static str net_mcast6iface;
//
static float net_dropsim = 0.0f;
...
static ipv6_mreq curgroup;
// And the currently bound address.
static sockaddr_in6 boundto;
...
void Sys_SendPacket( int length, const void *data, netadr_t to ) {
	...
	if(to.type == NA_MULTICAST6 && (net_enabled & NET_DISABLEMCAST))
		return;
	...
	*err = 0;
...
#ifdef IPV6_V6ONLY
	{
		...
		if(setsockopt(newsocket, IPPROTO_IPV6, IPV6_V6ONLY, (char *) &i, sizeof(i)) == SOCKET_ERROR)
		{
			...
			Com_Printf("WARNING: NET_IP6Socket: setsockopt IPV6_V6ONLY: %s\n", NET_ErrorString());
		}
	}
#endif
	...
	{
		if(!Sys_StringToSockaddr( net_interface, (sockaddr *)&address, sizeof(address), AF_INET6))
		{
			closesocket(newsocket);
			return INVALID_SOCKET;
		}
...
void NET_SetMulticast6(void)
{
	...
	if(net_mcast6addr.empty() || !Sys_StringToSockaddr(net_mcast6addr.c_str(), (sockaddr *) &addr, sizeof(addr), AF_INET6))
	{
		Com_Printf("WARNING: NET_JoinMulticast6: Incorrect multicast address given, "
			   "please set cvar %s to a sane value.\n", "net_mcast6addr");
		
		net_enabled |= NET_DISABLEMCAST;
	...
	if(!net_mcast6addr.empty())
	{
		curgroup.ipv6mr_interface = if_nametoindex(net_mcast6iface.c_str());
	}
	else
...
void NET_JoinMulticast6(void)
{
	...
	if(ip6_socket == INVALID_SOCKET || multicast6_socket != INVALID_SOCKET || (net_enabled & NET_DISABLEMCAST))
		return;
	...
	{
		if((multicast6_socket = NET_IP6Socket(net_mcast6addr.c_str(), ntohs(boundto.sin6_port), NULL, &err)) == INVALID_SOCKET)
		{
...
void NET_OpenSocks( int port ) {
	...
	h = gethostbyname( net_socksServer.c_str() );
	if ( h == NULL ) {
		err = socketError;
	...
	if ( *net_socksUsername.c_str() || *net_socksPassword.c_str() ) {
		rfc1929 = true;
	}
	else {
		rfc1929 = false;
	...
	if ( buf[1] == 2 ) {
		...
		// build the request
		ulen = strlen( net_socksUsername.c_str() );
		plen = strlen( net_socksPassword.c_str() );

		buf[0] = 1;		// username/password authentication version
		buf[1] = ulen;
		if ( ulen ) {
			memcpy( &buf[2], net_socksUsername.c_str(), ulen );
		}
		buf[2 + ulen] = plen;
		if ( plen ) {
			memcpy( &buf[3 + ulen], net_socksPassword.c_str(), plen );
		}
	...
	*(short *)&buf[8] = htons( (short)port );		// port
	...
	memset( ((sockaddr_in *)&socksRelayAddr)->sin_zero, 0, 8 );
...
void NET_OpenIP( void ) {
	...
	if(net_enabled & NET_ENABLEV6)
	{
		for( i = 0 ; i < 10 ; i++ )
		{
			ip6_socket = NET_IP6Socket(net_ip6.c_str(), port6 + i, &boundto, &err);
	...
	{
		for( i = 0 ; i < 10 ; i++ ) {
			ip_socket = NET_IPSocket( net_ip.c_str(), port + i, &err );
...
void NET_Event(fd_set *fdr)
{
	...
	while(1)
	{
		...
		if(net_dropsim > 0.0f && net_dropsim <= 100.0f)
			if(rand() < (int) (((double) RAND_MAX) / 100.0 * (double) net_dropsim))
				continue;          // drop this packet

		// sookee; I assume this is local server running
